...
def load_data(config: AblationConfig):
    ...
    train_texts = [t for t in ds["train"]["text"][:config.max_train_samples * 2] 
                   if t and len(t.strip()) > 20][:config.max_train_samples]
    eval_texts = [t for t in ds["validation"]["text"][:config.max_eval_samples * 2] 
                  if t and len(t.strip()) > 20][:config.max_eval_samples]
    ...
    def tokenize(texts):
        ...
        for text in texts:
            ...
            if len(tokens) > config.max_seq_len + 1:
                ...
                tokens = tokens + [tokenizer.pad_token_id] * (config.max_seq_len + 1 - len(tokens))
...
def train_and_evaluate(model, train_input, train_target, eval_input, eval_target, config):
...
def ablation_bipil(config: AblationConfig, train_input, train_target, eval_input, eval_target):
    ...
    for use_bipil in [False, True]:
        ...
        if metrics:
            ...
            print(f"    PPL: {metrics['perplexity']:.2f}, Acc: {metrics['accuracy']:.4f}")
...
def ablation_lambda(config: AblationConfig, train_input, train_target, eval_input, eval_target):
...
def ablation_expansion(config: AblationConfig, train_input, train_target, eval_input, eval_target):
...
def ablation_fusion(config: AblationConfig, train_input, train_target, eval_input, eval_target):
...
def format_results_table(results: List[Dict], key_column: str, title: str) -> str:
    ...
    for r in results:
        key_val = r.get(key_column.lower().replace(" ", "_"), r.get(key_column.lower(), "N/A"))
        lines.append(
...
def main():
    ...
    if "bipil" in all_results:
        summary += format_results_table(all_results["bipil"], "Variant", "BiPIL vs Single-PIL")
    
    if "lambda" in all_results:
        summary += format_results_table(all_results["lambda"], "Lambda", "Lambda Sensitivity")
    
    if "expansion" in all_results:
        summary += format_results_table(all_results["expansion"], "Expansion", "FFN Expansion Ratio")
    
    if "fusion" in all_results:
        summary += format_results_table(all_results["fusion"], "Fusion", "BiPIL Fusion Method")

...
def load_dataset(dataset_name: str, max_train: int, max_eval: int):
    ...
    try:
        ...
        train_texts = [t for t in ds["train"][text_col][:max_train * 2] if t and len(t.strip()) > 20][:max_train]
        eval_texts = [t for t in ds["validation"][text_col][:max_eval * 2] if t and len(t.strip()) > 20][:max_eval]
    ...
    with torch.no_grad():
        for i in range(num_batches):
            ...
            if mask.sum() > 0:
                ...
                total_correct_top5 += (top5 == target_flat[mask].unsqueeze(-1)).any(dim=-1).sum().item()
...
def train_baseline(
    ...
    for epoch in range(config.num_epochs):
        ...
        print(f"  Epoch {epoch + 1}: Loss={metrics['loss']:.4f}, PPL={metrics['perplexity']:.2f}, Acc={metrics['top1_accuracy']:.4f}")

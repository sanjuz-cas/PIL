...
class PILVelocityField:
    ...
    def _time_embedding(self, t: np.ndarray) -> np.ndarray:
        ...
        freqs = np.exp(
            -np.log(10000) * np.arange(half_dim) / half_dim
        ).astype(np.float32)
    ...
    def fit(
        ...
        if use_low_rank is None:
            use_low_rank = self.config.use_low_rank or N > self.config.low_rank_threshold
...
class BiPILVelocityField:
    ...
    def _time_embedding(self, t: np.ndarray) -> np.ndarray:
        ...
        half_dim = self.time_embed_dim // 2
        freqs = np.exp(-np.log(10000) * np.arange(half_dim) / half_dim).astype(np.float32)

...
class PILFlowMatching:
    ...
    def fit(
        ...
        if verbose:
            print(
                f"PIL-Flow Training: {N} samples, {len(self.train_timesteps)} timesteps"
            )
        ...
        for i, t in enumerate(self.train_timesteps):
            ...
            self._fit_history.append({"timestep_idx": i, "t": float(t), **fit_stats})

            if verbose and (i + 1) % max(1, len(self.train_timesteps) // 10) == 0:
                print(
                    f"  Timestep {i + 1}/{len(self.train_timesteps)}: "
